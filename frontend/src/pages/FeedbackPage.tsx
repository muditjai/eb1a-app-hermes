        ...
        >
          <Reveal visible={buyer.answer === "yes"}>
            <SliderField
              ...
              onChange={(amount) => {
                updateBuyer({ amount });
                capture("survey_slider_changed", { question: "buyer", amount });
              }}
            />
          </Reveal>

          <Reveal visible={buyer.answer === "no"}>
            <CommentField id="buyer-comment" value={buyer.comment} onChange={(comment) => updateBuyer({ comment })} />
          </Reveal>
        </QuestionPanel>
          ...
          <Reveal visible={contributor.answer === "yes"}>
            <SliderField
              ...
              onChange={(amount) => {
                ...
                capture("survey_slider_changed", { question: "contributor", amount });
              }}
            />
          </Reveal>

          <Reveal visible={contributor.answer === "no"}>
            <CommentField id="contributor-comment" value={contributor.comment} onChange={(comment) => updateContributor({ comment })} />
          </Reveal>
        </QuestionPanel>
...
function QuestionPanel({ number, question, answer, yesLabel, noLabel, onAnswer, children }: QuestionPanelProps) {
  return (
    <section className="question-panel">
      ...
      <div className="choice-row" role="group" aria-label={question}>
        <button className={answer === "yes" ? "choice selected" : "choice"} type="button" aria-pressed={answer === "yes"} onClick={() => onAnswer("yes")}>
          {yesLabel}
        </button>
        <button className={answer === "no" ? "choice selected" : "choice"} type="button" aria-pressed={answer === "no"} onClick={() => onAnswer("no")}>
          {noLabel}
        </button>
      </div>
...
function SliderField({ id, label, value, min, max, step, suffix, onChange }: SliderFieldProps) {
...
function CommentField({ id, value, onChange }: CommentFieldProps) {
  return (
    <label className="comment-field" htmlFor={id}>
      ...
      <textarea id={id} value={value} onChange={(event) => onChange(event.target.value)} placeholder="What would need to change?" rows={4} />
    </label>
  );
}
